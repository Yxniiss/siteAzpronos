(() => {
    ...
    const steps = [
        ...
        {
            slug: 'installation',
            kicker: '04 — INSTALLE SHUFFLE SUR TON ÉCRAN D’ACCUEIL',
            title: 'Installer le site sur l’écran d’accueil',
            text: 'Ajoute Shuffle à l’écran d’accueil de ton iPhone pour l’ouvrir ensuite comme une application.',
            imageLabel: 'Installation de Shuffle sur l’écran d’accueil',
            imageSrc: 'assets/tutorial-step-3-code.png',
            caption: 'Cette installation se fait directement depuis Safari, sans passer par l’App Store.',
            tasks: [
                'Ouvre le site Shuffle dans Safari.',
                'Appuie sur l’icône Partager (carré avec une flèche vers le haut).',
                'Fais défiler les options.',
                'Sélectionne « Sur l’écran d’accueil ».',
                'Confirme avec « Ajouter ».',
                'L’icône du site apparaît ensuite avec les autres applications.'
            ],
            tip: 'Tu pourras ouvrir Shuffle directement depuis ton écran d’accueil, comme n’importe quelle application.',
            actions: [
                { type: 'button', label: 'J’AI INSTALLÉ SHUFFLE →', action: 'next', style: 'tutorial-primary-button' }
            ]
        },
        {
            slug: 'wallet',
            kicker: '05 — CONFIGURE TON PORTEFEUILLE',
            title: 'Configure ton portefeuille',
            text: 'La plateforme peut demander certaines informations personnelles pour finaliser la configuration de ton compte.',
            imageLabel: 'Portefeuille et configuration du compte',
            imageSrc: 'assets/tutorial-step-4-deposit.png',
            caption: 'Utilise uniquement les informations réelles et exactes demandées par la plateforme.',
            tasks: [
                'Clique sur « Portefeuille ».',
                'Renseigne les informations demandées par la plateforme avec tes informations réelles et exactes.',
                'Vérifie attentivement les informations saisies.',
                'Valide la configuration.'
            ],
            tip: 'Les informations demandées peuvent varier selon le compte et la plateforme.',
            actions: [
                { type: 'button', label: 'MON PORTEFEUILLE EST CONFIGURÉ →', action: 'next', style: 'tutorial-primary-button' }
            ]
        },
        {
            slug: 'deposit',
            kicker: '06 — EFFECTUE TON PREMIER DÉPÔT',
            ...
            tasks: [
                ...
                'On va te demander nom / prénom / adresse / pays Tu peux choisir n’importe quel pays et mettre une adresse au hasard aucune vérification n’est faite ✅',
        ...
        },
        {
            ...
            supplemental: {
                name: 'AZZINFOS',
                text: 'Retrouve toutes les informations et actualités concernant nos offres sur AZZINFOS.'
            },
            actions: [
        ...
        {
            slug: 'ranking',
            kicker: '08 — GRIMPE DANS LE CLASSEMENT',
        ...
        {
            slug: 'community',
            kicker: '09 — REJOINS LA COMMUNAUTÉ',
    ...
    function renderStep(index) {
        ...
        const supplementalContainer = document.querySelector('.tutorial-step-supplemental');
        ...
        if (step.tip) {
            ...
            tip.innerHTML = `<strong>${step.slug === 'wallet' ? '💡 IMPORTANT' : '💡 À retenir'}</strong><p>${step.tip}</p>`;
            tipContainer.appendChild(tip);
        }
        if (step.supplemental) {
            supplementalContainer.innerHTML = `<p class="tutorial-supplemental-name">${step.supplemental.name}</p><p class="tutorial-supplemental-text">${step.supplemental.text}</p>`;
        }
        ...
        step.actions.forEach((action) => {
            ...
            if (action.type === 'link') {
                ...
                if (href) {
                    ...
                    element.title = 'Cette destination sera disponible prochainement.';
                }
            } else {
                element = document.createElement('button');
                element.type = 'button';
            ...
            .then((data) => {
                ...
                document.querySelectorAll('[data-link-key]').forEach((link) => {
                    ...
                    if (href) {
                        ...
                        link.title = 'Cette destination sera disponible prochainement.';
                    }
                });
    ...
    function hydrateFinalActionLinks() {
        document.querySelectorAll('[data-final-link-key]').forEach((control) => {
            const href = config[control.dataset.finalLinkKey];
            if (href) {
                control.href = href;
                ...
                control.title = 'Le canal AZZINFOS arrive bientôt.';
            }
        });
    }
